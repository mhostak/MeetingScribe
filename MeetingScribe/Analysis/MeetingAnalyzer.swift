...
struct MeetingAnalyzer: Sendable {
    ...
    private func transcriptChunks(from segments: [TranscriptSegment]) throws -> [String] {
    ...
    private func chunk(_ values: [String]) throws -> [String] {
        var chunks: [String] = []
        var current: [String] = []
        var currentLength = 0

        for value in values {
            guard value.count <= maxInputCharacters else {
                throw AnalysisError.transcriptChunkTooLarge
            }
            let addedLength = value.count + (current.isEmpty ? 0 : 1)
            if !current.isEmpty, currentLength + addedLength > maxInputCharacters {
