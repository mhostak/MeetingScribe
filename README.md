...
- Finalization chooses the earliest plausible captured track start as the timeline origin. A microphone start more than 60 seconds from the system-audio host-time is treated as implausible and omitted from the shared timeline. Because capture always starts system audio first, a microphone start more than 5 seconds *earlier* than system audio is clock skew rather than a head start: that track is still finalized, but it does not set the origin, and the recording records a warning. A track's relative offset is `max(0, trackStart - timelineOrigin)`; the converter does not insert leading silence. A track whose presentation timestamps span more time than its file contains — a gap opened by rebuilding the audio engine after a route change — is reported as a warning, because nothing in the output represents that gap.
- Tracks present in the selected recording mode are transcribed sequentially, each in its own short-lived helper process. The application process never creates an inference manager, so killing the helper releases the model even mid-prediction, and the roughly half a gigabyte of Core ML weights is not held across the gap between two tracks. The second load is cheap because macOS caches the compiled model outside the process: only the first load after an install pays the compilation, measured at about half a minute, and every load after that at a fraction of a second. A lightweight engine-neutral energy gate rejects completely silent or negligible tracks before inference; meaningful tracks are passed to Parakeet unchanged, preserving their original timeline.
...
The Recordings overview can delete one whole recording through the trash button at the right of its row, or the same item in its context menu. The confirmation names the session folder and its size, and states that the recorded audio, transcript, AI analysis, notes, and processing logs all go with it. A Markdown note exported outside the session folder is a separate checkbox in the same confirmation, because it may have been edited in an Obsidian vault since the export; a note exported into the session folder goes with the folder and is not offered twice. Both items are moved to the Trash rather than unlinked, so Finder can put them back. Deletion is refused while the recording is being captured, while it holds a pending or running processing job, during reprocessing, during audio cleanup, and for any folder a live process still owns. The paths are re-resolved from the recordings root immediately before the move, and an exported note is only touched when it is still a plain `.md` file under the name the manifest recorded.
