...
The current standard SwiftPM baseline contains 133 tests: 124 pass and 9 hardware-, credential-, model-, session-, or fixture-dependent tests skip unless explicitly enabled. The complete signed Xcode app/test scheme also passes. The Xcode app and test targets use Swift 6 with complete strict-concurrency checking. GitHub Actions runs both the SwiftPM suite and the shared Xcode scheme on a pinned `macos-26` runner, so the SwiftUI application layer cannot be skipped by a core-only build. In addition, the opt-in long-session test has been executed successfully against a generated one-hour, approximately 58 MB PCM stream. These automated counts describe the current working tree; release acceptance still requires the manual hardware matrix below.
...
A second 62-minute signed-build run validated the optimized pipeline end to end. Transcription completed in 11 minutes 19 seconds: 8 minutes 7 seconds for the highly active system track and 3 minutes 11 seconds for the microphone track, which skipped 2,781 seconds of inactive input. Across 58 one-minute resource samples, CPU averaged 4.06%, RSS stayed between approximately 34 and 48 MB, and physical footprint changed from 39 MB to 40 MB. Both CAF files grew continuously, more than 8.8 GiB remained free, finalization and export completed without warnings, and no artifact was deleted. The complete phase 10 hardware and application matrix is accepted; detailed evidence is in [the phase 10 results](docs/phase-10-results.md).
