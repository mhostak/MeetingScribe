...
- System and microphone tracks are transcribed sequentially. One loaded FluidAudio manager is reused between them and explicitly released after the complete session. A lightweight engine-neutral energy gate rejects completely silent or negligible tracks before inference; meaningful tracks are passed to Parakeet unchanged, preserving their original timeline.
...
**Phase A is complete.** The consent-first event and participant snapshot flow described below is implemented and validated. **Phase B remains intentionally deferred, but its speaker prerequisite is now implemented:** MeetingScribe diarizes system audio, persists stable user-managed speaker identities, and regenerates derived output without retranscribing audio. Phase B can next add explicit, confirmed attendee-to-speaker mapping. The delivery boundary is recorded in [Apple Calendar and participants](docs/apple-calendar-participants.md), and the completed prerequisite plus remaining release-quality gates are documented in [Speaker recognition and management](docs/speaker-recognition-management-plan.md).
...
Parakeet is the only transcription runtime, and offline `community-1` is connected to normal post-recording processing for anonymous system-speaker clusters. Missing or failed diarization falls back to deterministic continuous-utterance grouping. Old transcript and turn-artifact JSON remains readable and exportable without an old engine or model. The completed implementation and remaining labeled/manual release gates are documented in [the FluidAudio-only migration plan](docs/fluid-audio-migration-plan.md).
