...
When meeting notes exist, they are attached to every analysis request as an outline that the provider must not rewrite. The custom prompt can reference them with `{{user_notes}}`. Notes are sent to the provider only when AI analysis is enabled.
